...
public class MainWindow : Window
{
    ...
    private List<ResultItem> _combinedResults = new();
    ...
    private async Task ExecuteSearchAsync(int page = 1)
    {
        ...
        if (_pageInput != null) _pageInput.Text = _currentPage.ToString();
        Application.Refresh();
        ...
        try
        {
            ...
            if (cached != null)
            {
                ...
                {
                    _combinedResults.Add(new ResultItem { ImdbId = c.ImdbId, DisplayText = $"[CUSTOM] [{c.Type?.ToString() ?? "Unknown"}] {c.Title} ({c.Year}) ID: {c.ImdbId}", Details = c });
                }
            }

            if (_searchTypeGroup.SelectedItem == 0) // Title
            {
                ...
                if (response != null && response.Response && response.Results != null)
                {
                    ...
                    var filteredResults = response.Results.Where(r => r.ImdbId != specificMovieId).Select(r => new MovieDetails { ImdbId = r.ImdbId, Title = r.Title, Year = r.Year, Type = r.Type, PosterUrl = r.PosterUrl, IsDetailed = false }).ToList();
            ...
            // Exclude API results that are already in the cached results based on IMDB ID
        ...
        {
            Application.MainLoop.Invoke(() => {
                if (ex is System.Net.Http.HttpRequestException httpEx && httpEx.StatusCode == System.Net.HttpStatusCode.NotFound)
                {
                    _statusLabel.Text = "No results found";
                    _resultsList.SetSource(_combinedResults.Select(r => r.DisplayText).ToList());
                }
